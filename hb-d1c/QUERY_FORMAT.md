...
When `emit_migrations = true`, `out_dir/migrations.rs` embeds that ordered set as `MIGRATIONS`. Each entry exposes the relative path as `id`, a Rust string literal snapshot as `sql`, and a `sha256:<hex>` checksum computed from the same snapshot. Editing a migration without regenerating cannot combine new compile-time SQL with an old generated checksum. An ID is immutable; renaming a migration is removal plus addition. Generated code does not execute migrations or own state. The application must persist applied IDs and checksums, reject changed content for an applied ID, serialize migrators, and define transaction and recovery policy.
