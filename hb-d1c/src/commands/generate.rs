...
pub fn run(conn: &Connection, config: &D1CConfig) -> Result<()> {
    ...
    for (file_name, file_queries) in queries_by_file {
        ...
        if file_name == root_module_stem {
            anyhow::bail!("SQL file cannot be named '{file_name}.sql' as it conflicts with the root module name");
        }
        ...
        fs::write(submodule_dir.join(format!("{file_name}.rs")), formatted)?;
    ...
    for mod_name in modules {
        root_content.push_str(&format!("pub mod {mod_name};\n"));
    }
